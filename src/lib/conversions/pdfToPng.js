...
export async function pdfToPng(file, options = { scale: 2.0, onProgress: null }) {
  ...
  if (imageBlobs.length === 1) {
    return { isZip: false, blob: imageBlobs[0].blob, filename: `${file.name.replace(/\.pdf$/i, '')}-page-1.png` };
  }

  const zip = new JSZip();
  const baseName = file.name.replace(/\.pdf$/i, '');
  imageBlobs.forEach((item) => {
    zip.file(`${baseName}-page-${item.pageNum}.png`, item.blob);
  });

  const zipBlob = await zip.generateAsync({ type: 'blob' });
  return { isZip: true, blob: zipBlob, filename: `${baseName}-png-pages.zip` };
}
